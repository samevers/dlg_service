...
const static string ALL_WORDLIST_FILE="vrqo_VRwordlist";
...
bool QueryMatch::LoadWordList(const char *allWordlist)
{
		...
		else
		{
			...
			while(getline(finOne, keyLine)){
				...
				if(string::npos == pos){
					...
					boost::algorithm::split(tagKeyVec, keyLine, boost::is_any_of("="));
					if(2 != tagKeyVec.size())
					{
						cerr << "ERROR: "<< oneFilename  << " bad format! data:" << keyLine << "\t" << tagKeyVec.size()  << endl;
						continue;
					}
					string tempStr = tagKeyVec[0];
					
					trim(tempStr);
					string tag = tempStr;
					if (tagKeyMapIndex.find(tempStr) == tagKeyMapIndex.end())
					{
						tagKeyMapIndex[tempStr] = index;
						key_word_tag[index] = tempStr;
						index = index + 1;
					}
					vector<string> tagShowVec;
					boost::algorithm::split(tagShowVec, tagKeyVec[1], boost::is_any_of("|"));
					for(int i = 0; i < tagShowVec.size();i++)
					{
					...
					//cerr << "ERROR: "<< oneFilename  << " bad format! data:" << keyLine << endl;
					//continue;
				}
				else
				{
					string keyword = keyLine.substr(0, pos);
					trim(keyword);
					string tag = keyLine.substr(pos+1);
					trim(tag);
					string tempStr = tag;
					if (tagKeyMapIndex.find(tempStr) == tagKeyMapIndex.end())
					{
	...
	std::ifstream finRule(allRuleDir.c_str());
	if(!finRule)
	{
		cerr<<"can't open map lexion: "<<allWordlistDir<<endl;
		return false;
	...
	while(getline(finRule,line)){
		...
		trim(keyword);
		string tagComData = line.substr(pos+1);
		trim(tagComData);
		ruleMapRes[keyword] = tagComData;
		vector<string> eleTag;
		boost::algorithm::split(eleTag, tagComData, boost::is_any_of( "|"));
		if(7 != eleTag.size())
			continue;
		string tag = eleTag[0];
		string tagTmp = tag + "_key";

		//boost::algorithm::split(tagnames, keyword, boost::is_any_of( "|"));
		if(keyword.find("*") != string::npos)
		{
			allshowKey.insert(tag);
		}
		if(keyMapRule.find(tagTmp) != keyMapRule.end())
		{
			keyMapRule[tagTmp].push_back(keyword);
		}
		else
		{
...
bool QueryMatch::MakeTrie(map<string, bitset<MAXSBIT> > &keyMapBitTmp)
{
	vector<const char *> da_key_vect;
	...
	for (mit = keyMapBitTmp.begin(); mit != keyMapBitTmp.end(); mit++)
	{
		...
		bitset<MAXSBIT> bitTagTmp = mit->second;
		da_key_vect.push_back(key.c_str());
		string tempStr = bitTagTmp.to_string();
		if (mapBitTmp.find(tempStr) == mapBitTmp.end())
		{
			vecBit.push_back(bitTagTmp);
			mapBitTmp.insert(make_pair< string , int32_t>(tempStr, index));
			mapBitTmp[tempStr] = index;
			index = index + 1;
		}

		da_value_vect.push_back(mapBitTmp[tempStr]);
	}
	if(daKeyWorList.build(da_key_vect.size(),&da_key_vect[0], 0, &da_value_vect[0], NULL) != 0)
	{
...
bool QueryMatch::classifyQuery(const char *oriQuery, const char* query, int querylen, vector<ResultData> &vecType, long long request_id, analysis::QuerySegmentor *wordseg , analysis::TGraph *annseg, int modeFlag)
{
	...
	//int len = ec->t2sgchar(query, (gchar_t*)buffer, MAXSTRLEN/2, true);
...
bool QueryMatch::bj2qj(string & strTmp)
{
	...
	char buffer[MAXSTRLEN];
	//memset(buffer, 0,  MAXSTRLEN);
	//int len = ec->t2sgchar(strTmp.c_str(), (gchar_t*)buffer, MAXSTRLEN/2, true);
...
void QueryMatch::preconditioning(string &oriQuery, vector<PreInfo> &numberType)
{
	...
	//bool search_flag_number = boost::regex_search(oriQuery, what1, *fNumberpattern);
	string pnumber;
