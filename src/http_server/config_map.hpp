                    ...
                    if (line.length() && *(head = line.begin()) == '=')
                    {
                        ...
                        if (line.length() && *(head = line.begin()) == '"')
                        {
                            ...
                            if (tail != line.end()/* && tail - head != 1*/)
                            {
                                ...
                                set_value(key.c_str(), value.c_str());		// 插入 parameter 名称和 值
                            }
                        }
                    }
                }
...
{
	// 这个函数的目的是将 HTTP/Parameter 这种section 插入到链表
